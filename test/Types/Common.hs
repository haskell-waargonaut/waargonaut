...
indentImage = do
  ...
  pjson imageJson
  where
    pjson = TextIO.putStrLn . TextL.toLazyText . waargonautBuilder wsBuilder
...
parseBS = SD.parseWith AB.parseOnly W.parseWaargonaut

parseText :: Text -> Either DecodeError Json
parseText = SD.parseWith AT.parseOnly W.parseWaargonaut
